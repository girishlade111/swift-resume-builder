/**
 * ResumeForm — accordion-based form with all resume sections.
 */
...
export default function ResumeForm() {
  ...
  return (
    <div className="space-y-4">
      ...
      <Accordion type="multiple" defaultValue={['personal', 'summary', 'experience', 'education', 'projects', 'skills']} className="space-y-2">
        {/* Personal Info */}
              ...
              <div key={exp.id} className="space-y-3 rounded-lg border p-3 relative">
                ...
                <div className="grid gap-3 sm:grid-cols-2 pr-8">
                  ...
                  <div className="space-y-1.5">
                    ...
                    <Input value={exp.isCurrent ? 'Present' : exp.endDate} disabled={exp.isCurrent} onChange={e => ctx.updateExperience(exp.id, 'endDate', e.target.value)} placeholder="Dec 2024" />
            ...
            <div className="flex flex-wrap gap-1.5">
              {resume.skills.map((skill) => (
                <span key={skill} className="inline-flex items-center gap-1 rounded-full bg-primary/10 px-2.5 py-1 text-xs font-medium text-primary">
                  {skill}
                  <button onClick={() => removeSkill(skill)} className="ml-0.5 hover:text-destructive">&times;</button>
                </span>
              ))}
            </div>
            <Input
        ...
        <AccordionItem value="extras" className="rounded-lg border bg-card px-4">
          ...
          <AccordionContent className="pb-4 space-y-3">
            <Field label="Certifications">
              <Textarea value={resume.extras.certifications} onChange={e => ctx.updateExtras('certifications', e.target.value)} placeholder="AWS Cloud Practitioner (2024)&#10;Google UX Design Certificate" rows={2} />
            </Field>
            <Field label="Languages">
              <Textarea value={resume.extras.languages} onChange={e => ctx.updateExtras('languages', e.target.value)} placeholder="English (Fluent), Hindi (Native)" rows={2} />
            </Field>
            <Field label="Achievements">
              <Textarea value={resume.extras.achievements} onChange={e => ctx.updateExtras('achievements', e.target.value)} placeholder="Winner, Smart India Hackathon 2023" rows={2} />
            </Field>
          </AccordionContent>
        </AccordionItem>
      </Accordion>
