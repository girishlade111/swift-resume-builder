...
export function PdfClassic({ data }: { data: ResumeData }) {
  ...
  const SectionTitle = ({ children }: { children: React.ReactNode }) => (
    <View style={classicS.sectionWrap}><View style={classicS.sectionDash} /><Text style={classicS.section}>{children}</Text><View style={classicS.sectionRule} /></View>
  );
  return (
    <Document><Page size="A4" style={classicS.page}>
      ...
      <View style={classicS.body}>
        {summary ? <><SectionTitle>Professional Summary</SectionTitle><Text style={{ fontStyle: 'italic', color: CL.muted, lineHeight: 1.75, paddingLeft: 14 }}>{summary}</Text></> : null}
        {experience.length > 0 && <><SectionTitle>Professional Experience</SectionTitle>{experience.map(exp => (
          <View key={exp.id} style={{ marginBottom: 14 }}>
            <View style={classicS.row}><Text style={classicS.entryTitle}>{exp.role}</Text><Text style={classicS.meta}>{exp.startDate}{exp.startDate && (exp.isCurrent ? ' – Present' : exp.endDate ? ` – ${exp.endDate}` : '')}</Text></View>
        ...
        {education.length > 0 && <><SectionTitle>Education</SectionTitle>{education.map(edu => (
          <View key={edu.id} style={{ marginBottom: 10 }}>
            <View style={classicS.row}><Text style={classicS.entryTitle}>{edu.degree}{edu.fieldOfStudy ? ` in ${edu.fieldOfStudy}` : ''}</Text><Text style={classicS.meta}>{edu.startYear}{edu.endYear ? ` – ${edu.endYear}` : ''}</Text></View>
        ...
        {skills.length > 0 && <><SectionTitle>Skills & Expertise</SectionTitle><View style={{ flexDirection: 'row' as const, flexWrap: 'wrap' as const }}>{skills.map((s, i) => <Text key={i} style={classicS.skillTag}>{s}</Text>)}</View></>}
...
export function PdfCompact({ data }: { data: ResumeData }) {
  const { personal: p, summary, experience, education, projects, skills, extras } = data;
  const SectionTitle = ({ children }: { children: React.ReactNode }) => (
    <View style={compactS.sectionWrap}><View style={compactS.sectionDot} /><Text style={compactS.section}>{children}</Text><View style={compactS.sectionRule} /></View>
  );
  return (
    <Document><Page size="A4" style={compactS.page}>
      ...
      <View style={compactS.body}>
        ...
        {experience.length > 0 && <><SectionTitle>Experience</SectionTitle>{experience.map(exp => (
          <View key={exp.id} style={{ marginBottom: 9 }}>
            <View style={compactS.row}><Text style={compactS.entryTitle}>{exp.role}{exp.company ? <Text style={{ fontFamily: 'Helvetica', color: CP.muted }}> — {exp.company}</Text> : null}</Text><Text style={compactS.datePill}>{exp.startDate}{exp.startDate && (exp.isCurrent ? ' – Present' : exp.endDate ? ` – ${exp.endDate}` : '')}</Text></View>
        ...
        {education.length > 0 && <><SectionTitle>Education</SectionTitle>{education.map(edu => (
          <View key={edu.id} style={{ marginBottom: 6 }}>
            <View style={compactS.row}><Text style={compactS.entryTitle}>{edu.degree}{edu.fieldOfStudy ? ` in ${edu.fieldOfStudy}` : ''}</Text><Text style={compactS.meta}>{edu.startYear}{edu.endYear ? ` – ${edu.endYear}` : ''}</Text></View>
        ...
        {skills.length > 0 && <><SectionTitle>Technical Skills</SectionTitle><View style={{ flexDirection: 'row' as const, flexWrap: 'wrap' as const }}>{skills.map((s, i) => <Text key={i} style={compactS.skillTag}>{s}</Text>)}</View></>}
...
export function PdfLeftSidebar({ data }: { data: ResumeData }) {
  ...
  const MainSection = ({ children }: { children: React.ReactNode }) => (
    <View style={sidebarS.sectionWrap}><Text style={sidebarS.section}>{children}</Text><View style={sidebarS.sectionRule} /></View>
  );
  return (
    <Document><Page size="A4" style={sidebarS.page}>
      <View style={sidebarS.sidebar}>
        ...
        {extras.languages ? <><Text style={sidebarS.sidebarSection}>Languages</Text><Text style={sidebarS.sidebarText}>{extras.languages}</Text></> : null}
        {extras.certifications ? <><Text style={sidebarS.sidebarSection}>Certifications</Text><Text style={sidebarS.sidebarText}>{extras.certifications}</Text></> : null}
      </View>
      <View style={sidebarS.body}>
        ...
        {experience.length > 0 && <><MainSection>Experience</MainSection>{experience.map(exp => (
          <View key={exp.id} style={{ marginBottom: 14 }}>
            <View style={sidebarS.row}><Text style={sidebarS.entryTitle}>{exp.role}</Text><Text style={sidebarS.datePill}>{exp.startDate}{exp.startDate && (exp.isCurrent ? ' – Present' : exp.endDate ? ` – ${exp.endDate}` : '')}</Text></View>
        ...
        {education.length > 0 && <><MainSection>Education</MainSection>{education.map(edu => (
          <View key={edu.id} style={{ marginBottom: 10 }}>
            <View style={sidebarS.row}><Text style={sidebarS.entryTitle}>{edu.degree}{edu.fieldOfStudy ? ` in ${edu.fieldOfStudy}` : ''}</Text><Text style={sidebarS.meta}>{edu.startYear}{edu.endYear ? ` – ${edu.endYear}` : ''}</Text></View>
...
function ModernSectionTitle({ children }: { children: React.ReactNode }) {
  return <View style={modernS.sectionWrap}><View style={modernS.sectionBar} /><Text style={modernS.section}>{children}</Text></View>;
}

export function PdfModern({ data }: { data: ResumeData }) {
  const { personal: p, summary, experience, education, projects, skills, extras } = data;
  return (
    <Document><Page size="A4" style={modernS.page}>
      ...
      <View style={modernS.body}>
        {summary ? <><ModernSectionTitle>About Me</ModernSectionTitle><View style={modernS.summaryBox}><Text>{summary}</Text></View></> : null}
        {experience.length > 0 && <><ModernSectionTitle>Experience</ModernSectionTitle>{experience.map(exp => (
          <View key={exp.id} style={{ marginBottom: 14 }}>
            <View style={modernS.row}><Text style={modernS.entryTitle}>{exp.role}</Text><Text style={modernS.datePill}>{exp.startDate}{exp.startDate && (exp.isCurrent ? ' – Present' : exp.endDate ? ` – ${exp.endDate}` : '')}</Text></View>
            <Text style={modernS.meta}>{[exp.company, exp.location].filter(Boolean).join('  ·  ')}</Text>
            <BulletList items={exp.bulletPoints} color={MD.text} />
          </View>
        ))}</>}
        {education.length > 0 && <><ModernSectionTitle>Education</ModernSectionTitle>{education.map(edu => (
          <View key={edu.id} style={{ marginBottom: 8 }}>
            <View style={modernS.row}><Text style={modernS.entryTitle}>{edu.degree}{edu.fieldOfStudy ? ` in ${edu.fieldOfStudy}` : ''}</Text><Text style={{ fontSize: 9, color: MD.muted }}>{edu.startYear}{edu.endYear ? ` – ${edu.endYear}` : ''}</Text></View>
            <Text style={{ fontSize: 10, color: MD.muted }}>{edu.schoolName}{edu.grade ? ` — ${edu.grade}` : ''}</Text>
          </View>
        ))}</>}
        {projects.length > 0 && <><ModernSectionTitle>Projects</ModernSectionTitle>{projects.map(proj => (
        ...
        {skills.length > 0 && <><ModernSectionTitle>Technical Skills</ModernSectionTitle><View style={{ flexDirection: 'row' as const, flexWrap: 'wrap' as const }}>{skills.map((s, i) => <Text key={i} style={modernS.skillTag}>{s}</Text>)}</View></>}
...
const minimalS = StyleSheet.create({
  ...
  dividerLine: { height: 1, backgroundColor: MN.rule, marginTop: 10 },
...
export function PdfMinimal({ data }: { data: ResumeData }) {
  const { personal: p, summary, experience, education, projects, skills, extras } = data;
  const SectionTitle = ({ children }: { children: React.ReactNode }) => (
    <View><Text style={minimalS.sectionLabel}>{children}</Text><View style={minimalS.sectionBar} /></View>
  );
  return (
    <Document><Page size="A4" style={minimalS.page}>
      <View style={{ flexDirection: 'row' as const, alignItems: 'flex-start' as const, gap: 14 }}>
        ...
        <View><Text style={minimalS.name}>{p.fullName || 'Your Name'}</Text>{p.jobTitle ? <Text style={minimalS.jobTitle}>{p.jobTitle}</Text> : null}</View>
      </View>
      ...
      {skills.length > 0 && <><SectionTitle>Skills</SectionTitle><View style={{ flexDirection: 'row' as const, flexWrap: 'wrap' as const }}>{skills.map((s, i) => <Text key={i} style={minimalS.skillTag}>{s}</Text>)}</View></>}
...
export function PdfGeneric({ data, colors }: { data: ResumeData; colors: GenericColors }) {
  const { personal: p, summary, experience, education, projects, skills, extras } = data;
  const c = colors;
  ...
  const headerSubColor = isLight(c.headerBg) ? c.accent : c.accentLight;

  const s = StyleSheet.create({
    page: { fontFamily: 'Helvetica', fontSize: 10, color: c.text, lineHeight: 1.55 },
    header: { backgroundColor: c.headerBg, padding: '28 40 22', flexDirection: 'row' as const, alignItems: 'center' as const, gap: 16 },
    name: { fontSize: 22, fontFamily: 'Helvetica-Bold', color: headerTextColor, letterSpacing: 0.5 },
    jobTitle: { fontSize: 10, color: headerSubColor, marginTop: 3, fontFamily: 'Helvetica-Bold', letterSpacing: 1.5, textTransform: 'uppercase' as const },
    bar: { height: 3, backgroundColor: c.accent },
    body: { padding: '6 40 32' },
    secWrap: { flexDirection: 'row' as const, alignItems: 'center' as const, gap: 8, marginTop: 16, marginBottom: 8 },
    secBar: { width: 16, height: 3, backgroundColor: c.accent },
    secText: { fontSize: 9.5, fontFamily: 'Helvetica-Bold', textTransform: 'uppercase' as const, letterSpacing: 2.5, color: c.accent },
    secLine: { flex: 1, height: 1, backgroundColor: c.accentLight },
    title: { fontSize: 11, fontFamily: 'Helvetica-Bold', color: c.text },
    meta: { fontSize: 9, color: c.muted },
    row: { flexDirection: 'row' as const, justifyContent: 'space-between' as const, alignItems: 'center' as const },
    pill: { fontSize: 7.5, backgroundColor: c.accent, color: '#ffffff', padding: '1 7', fontFamily: 'Helvetica-Bold' },
    tag: { fontSize: 8.5, padding: '2 8', backgroundColor: c.accentLight, color: c.accent, fontFamily: 'Helvetica-Bold', marginRight: 4, marginBottom: 4 },
  });
  ...
  return (
    <Document><Page size="A4" style={s.page}>
        ...
        <View style={{ flex: 1 }}>
          ...
          <View style={{ flexDirection: 'row' as const, flexWrap: 'wrap' as const, gap: 8, marginTop: 6 }}>
            {[p.email, p.phone, p.location].filter(Boolean).map((ct, i) => (
              <Text key={i} style={{ fontSize: 8, color: isLight(c.headerBg) ? c.muted : 'rgba(255,255,255,0.65)' }}>{ct}</Text>
            ))}
          </View>
        </View>
      </View>
      <View style={s.bar} />
      <View style={s.body}>
        {summary ? <><Sec>Summary</Sec><Text style={{ color: c.muted, lineHeight: 1.75 }}>{summary}</Text></> : null}
        {experience.length > 0 && <><Sec>Experience</Sec>{experience.map(exp => (
          <View key={exp.id} style={{ marginBottom: 12 }}>
            ...
            <Text style={{ fontSize: 9.5, color: c.accent, marginTop: 1, fontFamily: 'Helvetica-Bold' }}>{[exp.company, exp.location].filter(Boolean).join(' · ')}</Text>
            <BulletList items={exp.bulletPoints} color={c.text} />
          </View>
        ))}</>}
        ...
        {projects.length > 0 && <><Sec>Projects</Sec>{projects.map(proj => (
          <View key={proj.id} style={{ marginBottom: 10 }}>
            ...
            {proj.techStack ? <Text style={{ fontSize: 8.5, color: c.accent, fontFamily: 'Helvetica-Bold' }}>{proj.techStack}</Text> : null}
            <BulletList items={proj.bulletPoints} color={c.text} />
          </View>
        ))}</>}
