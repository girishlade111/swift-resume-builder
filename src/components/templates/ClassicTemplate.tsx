/**
 * Classic template — traditional layout with serif headings and centered header.
 */
...
export default function ClassicTemplate({ data }: { data: ResumeData }) {
  ...
  return (
    <div style={{ fontFamily: 'Arial, Helvetica, sans-serif', fontSize: 11, lineHeight: 1.4, color: '#222', padding: '32px 36px' }}>
      ...
      {summary && <><SectionTitle>Summary</SectionTitle><p style={{ fontSize: 11, lineHeight: 1.5, margin: 0 }}>{summary}</p></>}
      ...
      {experience.length > 0 && <><SectionTitle>Experience</SectionTitle>{experience.map(exp => (
        <div key={exp.id} style={{ marginBottom: 10 }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'baseline' }}>
            ...
            <span style={{ fontSize: 10, color: '#666', flexShrink: 0, marginLeft: 8 }}>{exp.startDate}{exp.startDate && (exp.isCurrent ? ' – Present' : exp.endDate ? ` – ${exp.endDate}` : '')}</span>
          </div>
      ...
      {education.length > 0 && <><SectionTitle>Education</SectionTitle>{education.map(edu => (
        <div key={edu.id} style={{ marginBottom: 8 }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'baseline' }}>
            ...
            <span style={{ fontSize: 10, color: '#666', flexShrink: 0, marginLeft: 8 }}>{edu.startYear}{edu.startYear && edu.endYear ? ` – ${edu.endYear}` : ''}</span>
          </div>
